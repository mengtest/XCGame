...
local x_role_walk_to = class("x_role_walk_to", clsXUnit)

x_role_walk_to._default_args = default_args

function x_role_walk_to:ctor(args, xCtx, oXTree)
	clsXUnit.ctor(self, args, xCtx, oXTree)
	self._node_type = "x_role_walk_to"
end

function x_role_walk_to:OnStop()
	local atom_id = self:GetArgs().atom_id
	local role = self:GetXContext():GetXRole(atom_id)
	if role then role:CallRest() end
end

function x_role_walk_to:Proc(args, xCtx)
	...
	tmpChar:CallWalk(dx, dy, distance, function()
		self:on_event_point(XEventEnum.ec_xfinish)
	end)
	self:on_event_point(XEventEnum.ec_xstart)
	self:trigger_user_events()
